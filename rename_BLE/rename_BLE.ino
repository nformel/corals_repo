...
void setup(void)
{
  ...
  Serial.begin(9600);
  Serial.println(F("Rename BLE Script"));
  Serial.println(F("-------------------------------------"));
  ...
  //RENAME DEVICE
  //Display old name
  Serial.println("Rename BLE");
  Serial.println("Old name= ");
  ble.println("AT+GAPDEVNAME"); 
  ble.waitForOK();  // Check response status
  delay(1000);
  
  //Print new name to serial monitor and set in BLE
  Serial.println("New Name = RAPS7_BLE");
  ble.println("AT+GAPDEVNAME=RAPS7_BLE"); 
  ble.waitForOK();  // Check response status
  delay(1000);

  //Resest the BLE
  Serial.println("Reset to take effect");
  ble.println("ATZ"); 
  ble.waitForOK();  // Check response status
  delay(1000);

  //Print new name stored in BLE
  Serial.println("Verify. New Name on device = ");
  ble.println("AT+GAPDEVNAME"); 
  ble.waitForOK();  // Check response status   
}
